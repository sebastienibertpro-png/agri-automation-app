...
# --- GESTION DU RAPPORT DANS LE SESSION STATE ---
if "last_phyto_report" not in st.session_state:
    st.session_state["last_phyto_report"] = None
if "last_phyto_parcelle" not in st.session_state:
    st.session_state["last_phyto_parcelle"] = None
...
if st.button("🚀 Lancer le Contrôle Phyto", use_container_width=True, type="primary"):
    ...
    with st.spinner("🕵️‍♂️ L'IA inspecte les bases de données et vos traitements..."):
        # Pre-processing data for prompt
        produits_utilises = [str(p).strip() for p in df_parcelle_itk['Nom_Produit'].dropna().unique() if str(p).strip().lower() not in ['nan', 'none']]
        df_intrants = active_loader.get_intrants()
        df_usages = active_loader.get_usages_phyto()
        
        docs_intrants = ""
        docs_usages = ""
        if not df_intrants.empty and produits_utilises:
        ...
        prompt = f"""
=== PARCELLE ===
ID : {selected_parcelle} | Culture : {culture_en_place} | Surface : {surface_ha} ha
=== HISTORIQUE DES INTERVENTIONS (ITK) ===
{df_parcelle_itk[prompt_existing].to_csv(index=False)}
=== REFERENTIEL E-PHY (INTRANTS) ===
{docs_intrants}
=== REFERENTIEL E-PHY (USAGES) ===
{docs_usages}
Mission : Audit de conformité réglementaire.
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                st.session_state["last_phyto_report"] = full_response
                st.session_state["last_phyto_parcelle"] = selected_parcelle
                st.session_state["last_phyto_culture"] = culture_en_place
                break
...
if st.session_state["last_phyto_report"] and st.session_state["last_phyto_parcelle"] == selected_parcelle:
    st.divider()
    col_pdf, _ = st.columns([1, 1])
    with col_pdf:
        # Générer les bytes du PDF
        pdf_bytes = generate_ai_audit_pdf(
            st.session_state["last_phyto_parcelle"],
            st.session_state.get("last_phyto_culture", "Inconnue"),
            st.session_state["last_phyto_report"]
        )
        ...
        if pdf_bytes:
            st.download_button(
                ...
                data=pdf_bytes,
                file_name=f"Audit_Phyto_{selected_parcelle}_{selected_campaign}.pdf",
