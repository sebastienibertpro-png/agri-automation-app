...
try:
    if os.path.exists(logo_path):
        # Utiliser st.image avec use_column_width pour un effet bandeau
...
st.divider()
st.subheader("✍️ Saisie Rapide Multi-Interventions (Multi-Parcelles)")
...
with st.expander("Ouvrir le formulaire de saisie groupée", expanded=False):
    ...
    produits_data = []  # For Traitement & Fertilisation
    semis_data = {}     # For Semis
    recolte_data = {}   # For Récolte
    ...
    if nature_interv == "Traitement":
        ...
        if not df_intrants.empty and 'Nom_Produit' in df_intrants.columns:
            # Filter solely on phytos if Type column exists, else all
        ...
        # Extract N, P, K from REF_INTRANTS
    ...
    if submitted:
        if not selected_p_for_entry:
             ...
             for p in parcelles_data:
                  ...
                  # UID is unique per group or per row ? Generally per intervention group/machine pass
                  ...
                  base_row = {
                      ...
                      # Initialize empty for all specific columns
                  ...
                  if nature_interv == "Traitement":
                      ...
                      # Approximative quantity
                      if semis_data['unite'] == "Kg/ha":
                          qte = semis_data['densite'] * p['surface']
                      elif semis_data['unite'] == "Doses/ha":
                          qte = semis_data['densite'] * p['surface'] # Needs specific handling if Doses mean 50 000 grains
                      else: # Grains/m²
                          if semis_data['pmg'] > 0:
                               # (Grains/m2 * 10000) = Grains/ha.  Grains/ha * PMG(g) / 1000 / 1000 = Kg/ha
                      ...
                      # Insert associated products as separate rows
...
st.divider()
st.subheader("⚙️ Carnet d'Entretien Matériel")

try:
    with st.spinner("Chargement des matériels..."):
        df_materiels = loader.get_materiels()
        
    if df_materiels.empty:
        st.info("Aucun matériel trouvé dans REF_MATERIELS.")
    else:
        # Prepare dropdown options: "ID_Materiel - Marque Modele"
        materiel_options = []
        ...
        if not materiel_options:
            ...
            if st.button("📄 Générer Carnet d'Entretien"):
                ...
                with st.spinner(f"Récupération de l'historique pour {m_id}..."):
                    df_history = loader.get_maintenance_history(m_id)
